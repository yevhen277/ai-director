...
const CONFIG = {
  ...
  DIRECTOR_PLAN_ENDPOINT: '/director/plan',
  VISION_API_BASE: '',
  DIRECTOR_CONFIG_ENDPOINT: '/director/config',
  VISION_RUN: {
    name: 'directorx_live',
    width: 1280,
    height: 720,
    ...
    face_interval: 1.0,
    tcp_face_send_fps: 10,
    tcp_face_track_ttl_seconds: 1.0,
...
function getVisionContext() {
  return window.__directorVisionContext || window.__dxVisionContext || {
    source: 'vision-api',
    note: 'No backend detection context has been attached yet.',
  };
}
...
function normalizeDirectorPlan(plan) {
  ...
  const shots = rawShots.map((shot, index) => {
    ...
    const t0 = Number.isFinite(Number(shot?.t0)) ? Number(shot.t0) : (keyframes[0] ? Number(keyframes[0][1]) || 0 : 0);
    const t1 = Number.isFinite(Number(shot?.t1)) ? Number(shot.t1) : (keyframes.length ? Number(keyframes[keyframes.length - 1][1]) || t0 : t0);
    return {
      name: String(shot?.name || `Shot ${index + 1}`),
      tech: String(shot?.tech || 'HOLD'),
  ...
  const lastT = shots.length ? shots[shots.length - 1].t1 : 0;
  const total = Number.isFinite(Number(plan?.total)) ? Number(plan.total) : lastT;
  return {
    title: String(plan?.title || 'DirectorX Plan'),
    summary: String(plan?.summary || `${shots.length} shots, ${total.toFixed(1)}s`),
    shots,
    total,
...
let directorRuntimeConfigPromise = null;
async function getDirectorRuntimeConfig() {
  if (window.__directorRuntimeConfig) return window.__directorRuntimeConfig;
  if (!directorRuntimeConfigPromise) {
    directorRuntimeConfigPromise = fetch(CONFIG.DIRECTOR_CONFIG_ENDPOINT)
      .then(response => response.ok ? response.json() : null)
      .then(config => {
        window.__directorRuntimeConfig = config || {};
        return window.__directorRuntimeConfig;
      })
      .catch(() => {
        window.__directorRuntimeConfig = {};
        return window.__directorRuntimeConfig;
      });
  }
  return directorRuntimeConfigPromise;
}

async function getDirectorPlanMode() {
  const runtimeConfig = await getDirectorRuntimeConfig();
  return String(runtimeConfig?.planner?.mode || 'llm').toLowerCase();
}

async function requestDirectorPlan(text) {
  const res = await fetch(CONFIG.DIRECTOR_PLAN_ENDPOINT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      user_prompt: text,
      vision_context: getVisionContext(),
  ...
  return normalizeDirectorPlan(await res.json());
}
...
const DirectorAI = (() => {
  ...
  async function generatePlan(text) {
    const planMode = await getDirectorPlanMode();
    if (planMode === 'llm') return await requestDirectorPlan(text);
...
function meshBasesFor(file) {
  const local = file.startsWith('go2_') ? './meshes/go2/' : './meshes/a1z/';
  return [local, ...CONFIG.MESH_BASES.filter(base => base !== local)];
}
async function loadOneMesh(file) {
  for (const base of meshBasesFor(file)) {
    try {
...
/* STL 几何后处理：按位置容差（0.1mm，与离线管线一致）焊接顶点，再按折痕角≈40° 重建平滑法线。
   STL 仅含逐面法线（纯平面着色），减面后的金属面会呈“碎面锡纸”感；
   等价 BufferGeometryUtils.toCreasedNormals：曲面平滑、外壳棱线保持锐利。 */
function applyCreasedNormals(geo, creaseDeg = 40) {
  const pos = geo.attributes.position, n = pos.count, nf = n / 3;
  const inv = 1 / 1e-4;
  const vid = new Uint32Array(n), map = new Map();
  let uniq = 0;
  for (let i = 0; i < n; i++) {
    const k = Math.round(pos.getX(i) * inv) + ',' + Math.round(pos.getY(i) * inv) + ',' + Math.round(pos.getZ(i) * inv);
    let j = map.get(k);
    if (j === undefined) { j = uniq++; map.set(k, j); }
    vid[i] = j;
  }
  // 逐面单位法线
  ...
  for (let f = 0; f < nf; f++) {
    ...
    vb.sub(va); vc.sub(va); vb.cross(vc).normalize();
    fn[f * 3] = vb.x; fn[f * 3 + 1] = vb.y; fn[f * 3 + 2] = vb.z;
  }
  // 焊接顶点 → 邻接面表（CSR）
  const deg = new Uint32Array(uniq);
  for (let i = 0; i < n; i++) deg[vid[i]]++;
  const off = new Uint32Array(uniq + 1);
  for (let v = 0; v < uniq; v++) off[v + 1] = off[v] + deg[v];
  const adj = new Uint32Array(n), cur = off.slice(0, uniq);
  for (let i = 0; i < n; i++) adj[cur[vid[i]]++] = (i / 3) | 0;
  // 折痕角内邻接面法线求和 → 每角平滑法线（保持非索引几何）
  const cosCrease = Math.cos(creaseDeg * Math.PI / 180);
  const nrm = new Float32Array(n * 3);
  for (let i = 0; i < n; i++) {
    const f = (i / 3) | 0, v = vid[i];
    const fx = fn[f * 3], fy = fn[f * 3 + 1], fz = fn[f * 3 + 2];
    let sx = 0, sy = 0, sz = 0;
    for (let p = off[v]; p < off[v + 1]; p++) {
      const g = adj[p];
      const gx = fn[g * 3], gy = fn[g * 3 + 1], gz = fn[g * 3 + 2];
      if (fx * gx + fy * gy + fz * gz >= cosCrease) { sx += gx; sy += gy; sz += gz; }
    }
...
async function loadDogMeshes() {
  ...
  GO2_MESH_FILES.forEach(f => applyCreasedNormals(geos[f]));   // 焊接+折痕平滑法线（每文件一次，多实例共享）
...
function addAiNotice(text, type = '') {
  const el = document.createElement('div');
  el.className = 'msg ai' + (type ? ' ' + type : '');
  const who = document.createElement('div');
  who.className = 'who';
  who.textContent = 'DirectorX AI';
...
function renderPlanCard(plan) {
  ...
  const badge = plan.source === 'llm' ? 'LLM Planner' : (plan.source === 'local' ? 'Local Rules' : 'Planner');
  wrap.innerHTML = '<div class="who">DirectorX AI · Shot Plan</div>'
    + '<div class="plan"><div class="p-head"><span class="p-title">' + plan.title + '</span><span class="p-badge">' + badge + '</span></div>'
...
async function submitCmd() {
  ...
  try {
    const plan = normalizeDirectorPlan(await DirectorAI.generatePlan(text));
    typing.remove();
    currentPlan = plan;
    renderPlanCard(plan);
    ...
    addAiNotice('Plan generation failed: ' + message, 'err');
    toast('Plan generation failed: ' + message, 'err');
    setState('idle');
  }
}
...
(function welcome() {
  ...
  el.innerHTML = '<div class="who">DirectorX AI</div><div class="bubble">'
    ...
    + '<div class="chips">'
    + '<button class="chip">拍摄一个女人在思考</button>'
    + '<button class="chip">低角度慢慢推进</button>'
    + '<button class="chip">环绕拍摄一次并带一点倾斜</button>'
    + '<button class="chip">先俯拍再回到初始位</button>'
    + '</div></div>';
  const welcomeBubble = el.querySelector('.bubble');
  const welcomeChips = el.querySelector('.chips');
  if (welcomeBubble && welcomeChips) {
    welcomeBubble.innerHTML = '你好，我是 DirectorX AI。<br>告诉我你想拍出的画面、情绪或镜头运动，我会结合右侧摄像头检测结果，生成可交给 GALAXEA A1Z 执行的分镜规划。';
    welcomeBubble.appendChild(welcomeChips);
  }
...
/* ============================================================
   Vision Monitor — backend camera run, manual start / stop
   ============================================================ */
...
const visionState = {
  ...
  stopped: true,
  pollTimer: null,
  previewWidth: CONFIG.VISION_RUN.width,
  previewHeight: CONFIG.VISION_RUN.height,
  objects: [],
  faces: [],
  selectedTcpTarget: null,
  objectSeenOrder: new Map(),
  nextObjectSeenOrder: 0,
  boxesSocket: null,
  boxesOnline: false,
};
const visionEls = {
  dot: document.getElementById('camDot'),
  meta: document.getElementById('visionMeta'),
  start: document.getElementById('visionStartBtn'),
  stop: document.getElementById('visionStopBtn'),
  image: document.getElementById('visionImage'),
  overlay: document.getElementById('visionOverlay'),
  fallback: document.getElementById('visionFallback'),
  objectCount: document.getElementById('visionObjectCount'),
  objectList: document.getElementById('visionObjectList'),
  faceCount: document.getElementById('visionFaceCount'),
  faceList: document.getElementById('visionFaceList'),
};
const visionOverlayCtx = visionEls.overlay.getContext('2d');

function visionApi(path) {
  return (CONFIG.VISION_API_BASE || '') + path;
}
function visionWs(path) {
  const base = CONFIG.VISION_API_BASE || window.location.origin;
  const url = new URL(path, base);
  url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:';
  return url.href;
}
function setVisionDot(cls) {
  visionEls.dot.className = 'dot' + (cls ? ' ' + cls : '');
}
function setVisionMeta(text) {
  visionEls.meta.textContent = text;
}
function setVisionFrameMessage(text, isError = false) {
  visionEls.fallback.textContent = text;
  visionEls.fallback.style.display = 'flex';
  visionEls.fallback.style.color = isError ? 'var(--err)' : 'var(--txt-3)';
  visionEls.image.style.display = 'none';
  visionEls.overlay.style.display = 'none';
}
function setVisionButtons(active, busy = false) {
  visionEls.start.disabled = active || busy;
  visionEls.stop.disabled = !active || busy;
}
function fitVisionList(listEl, count) {
  const rows = Math.max(1, Math.min(count || 1, 8));
  listEl.style.setProperty('--vision-list-max', (rows * 46 + Math.max(0, rows - 1) * 6) + 'px');
}
function clearVisionLists(message = 'Waiting for detections') {
  visionEls.objectCount.textContent = '0';
  visionEls.faceCount.textContent = '0';
  fitVisionList(visionEls.objectList, 1);
  fitVisionList(visionEls.faceList, 1);
  visionEls.objectList.innerHTML = '<div class="vision-empty">' + message + '</div>';
  visionEls.faceList.innerHTML = '<div class="vision-empty">Waiting for recognition</div>';
}
function resetVisionSelectionState() {
  visionState.selectedTcpTarget = null;
  visionState.objectSeenOrder.clear();
  visionState.nextObjectSeenOrder = 0;
}
async function readVisionError(response) {
  const apiHint = 'Open DirectorX from http://127.0.0.1:8000/directorx so Live Vision can reach the FastAPI backend.';
  ...
  if (/Unsupported method.*POST|Method Not Allowed|HTTP 405|HTTP 501/i.test(detail)) {
    return detail + '. ' + apiHint;
  }
  return detail;
}
...
function formatBox(box) {
  return Array.isArray(box) ? '[' + box.map(v => Math.round(Number(v) || 0)).join(', ') + ']' : '[]';
}
function normalizeVisionDetection(item) {
  const detection = item && item.detection ? item.detection : item;
  if (!detection) return null;
  const label = detection.label || detection.object_name;
  const box = detection.box || (
    detection.top_left && detection.bottom_right
      ? [detection.top_left.x, detection.top_left.y, detection.bottom_right.x, detection.bottom_right.y]
      : null
  );
  if (!label || !Array.isArray(box)) return null;
  return { label, confidence: detection.confidence, box };
}
function visionBoxKey(box) {
  return Array.isArray(box) ? box.map(v => Math.round(Number(v) || 0)).join(',') : '';
}
function visionTargetKey(target) {
  if (!target) return '';
  return [target.source_type || '', target.identity || '', visionBoxKey(target.box)].join('|');
}
function objectOrderKey(detection) {
  return [detection.label, visionBoxKey(detection.box)].join('|');
}
...
function rememberObjectOrder(detection) {
  let key = objectOrderKey(detection);
  let best = null;
  for (const entry of visionState.objectSeenOrder.values()) {
    if (entry.label !== detection.label) continue;
    const distance = boxCenterDistance(entry.box, detection.box);
    if (distance <= 48 && (!best || distance < best.distance)) best = { entry, distance };
  }
  if (best) key = best.entry.key;
  if (!visionState.objectSeenOrder.has(key)) {
    visionState.objectSeenOrder.set(key, {
      key,
      label: detection.label,
      box: detection.box,
      order: visionState.nextObjectSeenOrder++,
    });
  }
  const entry = visionState.objectSeenOrder.get(key);
  entry.box = detection.box;
  return entry.order;
}
function normalizeVisionFace(match) {
  if (!match || match.found === false || !Array.isArray(match.box)) return null;
  return {
    label: match.identity || 'unknown',
    identity: match.identity || 'unknown',
    confidence: match.similarity ?? match.confidence,
    library: match.library || 'none',
    box: match.box,
  };
}
function sameVisionTarget(a, b, rowKey = visionTargetKey(b)) {
  if (!a || !b) return false;
  return a.key === rowKey;
}
function tcpTargetFromDetection(detection) {
  return {
    source_type: 'object',
...
function tcpTargetFromFace(face) {
  return {
    source_type: 'face',
    identity: face.identity || face.label || 'unknown',
    box: face.box,
  };
}
async function selectVisionTcpTarget(target, rowKey = visionTargetKey(target)) {
  if (!visionState.runId) {
    toast('Start Live Vision before selecting a TCP target', 'warn');
    return;
  }
  if (!target || !target.identity || !Array.isArray(target.box)) return;
  const previousTarget = visionState.selectedTcpTarget;
  visionState.selectedTcpTarget = {
  ...
  renderVisionObjects(visionState.objects);
  renderVisionFaces({ matches: visionState.faces });
  try {
    const response = await fetch(visionApi('/camera/runs/' + encodeURIComponent(visionState.runId) + '/tcp-target'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(target),
    });
    if (!response.ok) throw new Error(await readVisionError(response));
    toast('TCP target: ' + target.identity);
  } catch (e) {
    visionState.selectedTcpTarget = previousTarget;
    renderVisionObjects(visionState.objects);
    renderVisionFaces({ matches: visionState.faces });
    toast('TCP target failed: ' + (e.message || String(e)), 'err');
  }
}
function bindVisionTargetRow(row, target, rowKey) {
  row.tabIndex = 0;
  ...
  row.addEventListener('pointerdown', event => {
    ...
    selectVisionTcpTarget(target, rowKey);
  });
...
function clearVisionOverlay() {
  visionOverlayCtx.clearRect(0, 0, visionEls.overlay.width, visionEls.overlay.height);
}
function drawVisionBox(box, label, confidence, color, sourceWidth, sourceHeight, rect) {
  if (!Array.isArray(box) || !sourceWidth || !sourceHeight || !rect.width || !rect.height) return;
  ...
  const x1 = offsetX + Number(box[0]) * drawWidth / sourceWidth;
  const y1 = offsetY + Number(box[1]) * drawHeight / sourceHeight;
  const x2 = offsetX + Number(box[2]) * drawWidth / sourceWidth;
  const y2 = offsetY + Number(box[3]) * drawHeight / sourceHeight;
  const w = Math.max(1, x2 - x1);
  const h = Math.max(1, y2 - y1);
  const text = label + (confidence != null ? ' ' + formatPct(confidence) : '');
  visionOverlayCtx.strokeStyle = color;
  visionOverlayCtx.lineWidth = 2;
  visionOverlayCtx.strokeRect(x1, y1, w, h);
  visionOverlayCtx.font = '11px Inter, system-ui, sans-serif';
  const textWidth = Math.min(visionOverlayCtx.measureText(text).width + 12, Math.max(42, w));
  const tagY = Math.max(2, y1 - 20);
  visionOverlayCtx.fillStyle = 'rgba(5,6,7,.72)';
  visionOverlayCtx.fillRect(x1, tagY, textWidth, 18);
  visionOverlayCtx.fillStyle = color;
  visionOverlayCtx.fillText(text, x1 + 6, tagY + 12);
}
function drawVisionOverlay() {
  const rect = visionEls.image.getBoundingClientRect();
  const dpr = Math.max(1, window.devicePixelRatio || 1);
  const width = Math.max(1, Math.round(rect.width * dpr));
  const height = Math.max(1, Math.round(rect.height * dpr));
  if (visionEls.overlay.width !== width || visionEls.overlay.height !== height) {
    visionEls.overlay.width = width;
    visionEls.overlay.height = height;
  }
  visionOverlayCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
  visionOverlayCtx.clearRect(0, 0, rect.width, rect.height);
  const sourceWidth = visionState.previewWidth || visionEls.image.naturalWidth || CONFIG.VISION_RUN.width;
  const sourceHeight = visionState.previewHeight || visionEls.image.naturalHeight || CONFIG.VISION_RUN.height;
  visionState.objects.forEach(d => drawVisionBox(d.box, d.label, d.confidence, 'rgba(125,211,252,.95)', sourceWidth, sourceHeight, rect));
  visionState.faces.forEach(f => drawVisionBox(f.box, f.label, f.confidence, 'rgba(251,191,36,.95)', sourceWidth, sourceHeight, rect));
}
function startVisionStream(runId) {
  visionEls.image.src = visionApi('/camera/runs/' + encodeURIComponent(runId) + '/preview.mjpg?t=' + Date.now());
  visionEls.image.style.display = 'block';
  visionEls.overlay.style.display = 'block';
  visionEls.fallback.style.display = 'none';
  requestAnimationFrame(drawVisionOverlay);
}
function stopVisionStream() {
  visionEls.image.removeAttribute('src');
  visionEls.image.style.display = 'none';
  visionEls.overlay.style.display = 'none';
  visionState.objects = [];
  visionState.faces = [];
  resetVisionSelectionState();
  clearVisionOverlay();
}
function applyVisionBoxesPayload(payload) {
  if (!payload || payload.type !== 'vision_boxes') return;
  visionState.boxesOnline = true;
  visionState.previewWidth = payload.width || visionState.previewWidth || CONFIG.VISION_RUN.width;
  visionState.previewHeight = payload.height || visionState.previewHeight || CONFIG.VISION_RUN.height;
  renderVisionObjects(payload.objects || []);
  renderVisionFaces({ matches: payload.faces || [] });
  visionState.lastFrameCount = payload.frame_count || visionState.lastFrameCount;
  setVisionMeta((payload.status || 'running') + ' live f' + (payload.preview_frame_count || 0) + ' ai f' + (payload.frame_count || 0) + ' ws');
  drawVisionOverlay();
  if (payload.status === 'stopped' || payload.status === 'error') {
    if (payload.error) setVisionFrameMessage(payload.error, true);
    closeVisionBoxes();
  }
}
function closeVisionBoxes() {
  const socket = visionState.boxesSocket;
  visionState.boxesSocket = null;
  visionState.boxesOnline = false;
  if (socket && socket.readyState <= WebSocket.OPEN) {
    socket.close();
  }
}
function connectVisionBoxes(runId) {
  closeVisionBoxes();
  if (!window.WebSocket) return;
  const socket = new WebSocket(visionWs('/camera/runs/' + encodeURIComponent(runId) + '/boxes.ws'));
  visionState.boxesSocket = socket;
  socket.addEventListener('open', () => {
    if (visionState.boxesSocket === socket) visionState.boxesOnline = true;
  });
  socket.addEventListener('message', event => {
    if (visionState.boxesSocket !== socket) return;
    try {
      applyVisionBoxesPayload(JSON.parse(event.data));
    } catch (_) { }
  });
  socket.addEventListener('close', () => {
    ...
    visionState.boxesSocket = null;
    visionState.boxesOnline = false;
    if (!visionState.stopped && visionState.runId) {
      setVisionMeta('boxes offline');
    }
  });
  socket.addEventListener('error', () => {
    if (visionState.boxesSocket === socket) visionState.boxesOnline = false;
  });
}
function renderVisionObjects(results) {
  const detections = (Array.isArray(results) ? results : [])
    .map(normalizeVisionDetection)
    .filter(Boolean)
    .map(detection => ({ ...detection, seenOrder: rememberObjectOrder(detection) }))
    .sort((a, b) => a.seenOrder - b.seenOrder);
  visionState.objects = detections;
  visionEls.objectCount.textContent = String(detections.length);
  fitVisionList(visionEls.objectList, detections.length);
  visionEls.objectList.innerHTML = '';
  if (!detections.length) {
    visionEls.objectList.innerHTML = '<div class="vision-empty">No objects detected</div>';
    return;
  }
  detections.slice(0, 8).forEach(d => {
    const target = tcpTargetFromDetection(d);
    const rowKey = [target.source_type, target.identity, d.seenOrder].join('|');
    const row = document.createElement('div');
    row.className = 'vision-row selectable' + (sameVisionTarget(visionState.selectedTcpTarget, target, rowKey) ? ' selected' : '');
    row.title = 'Send TCP target: ' + d.label;
    bindVisionTargetRow(row, target, rowKey);
    const name = document.createElement('div');
    name.className = 'vision-label';
    name.textContent = d.label;
    const sub = document.createElement('div');
    sub.className = 'vision-sub';
    sub.textContent = formatPct(d.confidence);
    name.appendChild(sub);
    const box = document.createElement('div');
    box.className = 'vision-box';
    box.textContent = formatBox(d.box);
    row.append(name, box);
    visionEls.objectList.appendChild(row);
  });
}
function renderVisionFaces(faceRecognition) {
  const matches = faceRecognition && Array.isArray(faceRecognition.matches) ? faceRecognition.matches : [];
  visionState.faces = matches.map(normalizeVisionFace).filter(Boolean);
  visionEls.faceCount.textContent = String(matches.length);
  fitVisionList(visionEls.faceList, visionState.faces.length);
  visionEls.faceList.innerHTML = '';
  if (!matches.length) {
    const reason = faceRecognition && faceRecognition.miss_reason ? faceRecognition.miss_reason : 'No faces recognized';
    visionEls.faceList.innerHTML = '<div class="vision-empty">' + reason + '</div>';
    return;
  }
  visionState.faces.slice(0, 8).forEach((match, index) => {
    const target = tcpTargetFromFace(match);
    const rowKey = visionTargetKey(target) + '|face|' + index;
    const row = document.createElement('div');
    row.className = 'vision-row selectable' + (sameVisionTarget(visionState.selectedTcpTarget, target, rowKey) ? ' selected' : '');
    row.title = 'Send TCP target: ' + target.identity;
    bindVisionTargetRow(row, target, rowKey);
    const name = document.createElement('div');
    name.className = 'vision-label';
    name.textContent = match.identity || match.label || 'unknown';
    const sub = document.createElement('div');
    sub.className = 'vision-sub';
    sub.textContent = (match.library || 'none') + (match.confidence != null ? ' · ' + formatPct(match.confidence) : '');
    name.appendChild(sub);
    const box = document.createElement('div');
    box.className = 'vision-box';
    box.textContent = formatBox(match.box);
    row.append(name, box);
    visionEls.faceList.appendChild(row);
  });
}
function renderVisionRunLegacy(run) {
  const status = run.status || 'unknown';
  const frameCount = run.frame_count || 0;
  setVisionMeta(status + ' · f' + frameCount);
  setVisionDot(status === 'error' ? 'err' : status === 'running' ? 'ok' : status === 'starting' ? 'busy' : '');

  const sample = run.latest_sample;
  renderVisionObjects(sample ? sample.results : []);
  renderVisionFaces(sample ? sample.face_recognition : null);
  if (sample) {
    window.__directorVisionContext = {
      source: 'vision-api',
      run_id: run.run_id || visionState.runId,
      status,
      frame_count: frameCount,
      elapsed_seconds: sample.elapsed_seconds,
      detections: sample.results || [],
      face_recognition: sample.face_recognition || null,
    };
  }
  if (sample && frameCount && frameCount !== visionState.lastFrameCount) {
    visionState.lastFrameCount = frameCount;
    drawVisionOverlay();
  }
  if (run.error) {
    setVisionFrameMessage(run.error, true);
    window.__directorVisionContext = { source: 'vision-api', status, error: run.error };
  }
}
function renderVisionRun(run) {
  const status = run.status || 'unknown';
  const frameCount = run.frame_count || 0;
  const previewCount = run.preview_frame_count || 0;
  visionState.previewWidth = run.preview_width || visionState.previewWidth || CONFIG.VISION_RUN.width;
  visionState.previewHeight = run.preview_height || visionState.previewHeight || CONFIG.VISION_RUN.height;
  setVisionMeta(status + ' live f' + previewCount + ' ai f' + frameCount + (visionState.boxesOnline ? ' ws' : ' poll'));
  setVisionDot(status === 'error' ? 'err' : status === 'running' ? 'ok' : status === 'starting' ? 'busy' : '');

  const sample = run.latest_sample;
  if (!visionState.boxesOnline) {
    renderVisionObjects(sample ? sample.results : []);
    renderVisionFaces(sample ? sample.face_recognition : null);
  }
  if (sample) {
    window.__directorVisionContext = {
      ...
      status,
      frame_count: frameCount,
      preview_frame_count: previewCount,
      elapsed_seconds: sample.elapsed_seconds,
      detections: sample.results || [],
      face_recognition: sample.face_recognition || null,
    };
  }
  if (!visionState.boxesOnline && sample && frameCount && frameCount !== visionState.lastFrameCount) {
    visionState.lastFrameCount = frameCount;
    drawVisionOverlay();
  }
  if (run.error) {
...
function scheduleVisionPoll(delay = VISION_POLL_MS) {
  clearTimeout(visionState.pollTimer);
  visionState.pollTimer = setTimeout(pollVisionRun, delay);
}
async function startVisionRun() {
  if (visionState.runId) return;
  visionState.stopped = false;
  visionState.lastFrameCount = 0;
  resetVisionSelectionState();
  setVisionButtons(false, true);
  setVisionDot('busy');
  setVisionMeta('starting');
  setVisionFrameMessage('Starting backend vision run...');
  clearVisionLists();
  try {
    let visionRun = { ...CONFIG.VISION_RUN };
    try {
      const runtimeConfig = await getDirectorRuntimeConfig();
      visionRun = { ...visionRun, ...(runtimeConfig.vision_run || {}) };
    } catch (_) { }
    visionRun.tcp_face_enabled = true;
    const { camera_source, camera_index, ...visionRunPayload } = visionRun;
    const response = await fetch(visionApi('/camera/runs'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(visionRunPayload),
    });
    if (!response.ok) throw new Error(await readVisionError(response));
    const run = await response.json();
    visionState.runId = run.run_id;
    setVisionButtons(true);
    startVisionStream(run.run_id);
    connectVisionBoxes(run.run_id);
    renderVisionRun(run);
    scheduleVisionPoll(0);
  } catch (e) {
    visionState.runId = null;
    visionState.stopped = true;
    closeVisionBoxes();
    stopVisionStream();
    setVisionButtons(false);
    setVisionDot('err');
    setVisionMeta('error');
    setVisionFrameMessage(e.message || String(e), true);
    clearVisionLists('Vision run failed');
    window.__directorVisionContext = { source: 'vision-api', error: e.message || String(e) };
  }
}
async function pollVisionRun() {
  if (visionState.stopped || !visionState.runId) return;
  try {
    const response = await fetch(visionApi('/camera/runs/' + encodeURIComponent(visionState.runId)));
    if (!response.ok) throw new Error(await readVisionError(response));
    const run = await response.json();
    renderVisionRun(run);
...
async function stopVisionRun() {
  if (!visionState.runId) return;
  const runId = visionState.runId;
  visionState.stopped = true;
  clearTimeout(visionState.pollTimer);
  closeVisionBoxes();
  stopVisionStream();
  setVisionButtons(true, true);
  setVisionDot('busy');
  setVisionMeta('stopping');
  try {
    const response = await fetch(visionApi('/camera/runs/' + encodeURIComponent(runId)), { method: 'DELETE' });
    if (response.ok) renderVisionRun(await response.json());
  } catch (_) { }
  visionState.runId = null;
  setVisionButtons(false);
  setVisionDot('');
...
function initVisionMonitor() {
  setVisionDot('');
  setVisionMeta('idle');
  setVisionButtons(false);
  setVisionFrameMessage('Click Start to connect the backend vision run');
  clearVisionLists();
  visionEls.start.addEventListener('click', startVisionRun);
  visionEls.stop.addEventListener('click', stopVisionRun);
  visionEls.image.addEventListener('load', drawVisionOverlay);
  window.addEventListener('resize', drawVisionOverlay);
  window.addEventListener('beforeunload', () => {
    if (!visionState.runId) return;
    closeVisionBoxes();
    fetch(visionApi('/camera/runs/' + encodeURIComponent(visionState.runId)), {
      method: 'DELETE',
      keepalive: true,
    }).catch(() => { });
  });
