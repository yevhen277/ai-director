...
const info=(q)=>{const{tip,gaze}=fk(q);const d=sub(S,tip),dist=nrm(d);
 ...
 return{tip,dist,aim};};
function cost(q,T){const{tip,dist,aim}=info(q);const e=sub(tip,T);
 return 16*(e[0]**2+e[1]**2+e[2]**2)+aim*aim;}
function solve(T,seed){let best=seed.map((v,i)=>Math.min(LIM[i][1],Math.max(LIM[i][0],v))),bc=cost(best,T),step=0.3;
 ...
 return{best,bc};}
function report(name,T,seed,restarts=20){let best=null;
 ...
 console.log(name.padEnd(8),'['+b.map(x=>x.toFixed(3)).join(', ')+']','| aim',(aim*180/Math.PI).toFixed(1).padStart(5),'° perr',perr.toFixed(0).padStart(3),'mm distS',(dist*100).toFixed(1).padStart(5),'cm');
 return b;}
const home=report('home',[0,-0.155,0.16],[0,0.9,-1.1,0.9,0,0.35]);
const close=report('close',[0,-0.185,0.135],[0,0.5,-0.7,0.9,0,0.35]);
const far=report('far',[0,-0.125,0.20],[0,0.3,-1.2,1.1,0,0.35]);
const top=report('top',[0,-0.19,0.24],[0,0.9,-0.9,0.5,0,0.35]);
const low=report('low',[0,-0.17,0.08],[0,0.4,-0.9,1.0,0,0.35]);
console.log('--- orbit arc (10cm around S, h=15.5cm), J1 swept, J2-J5 solved ---');
let prev=home;
for(const th of[-0.45,-0.225,0,0.225,0.45]){
 const T=[Math.sin(th)*0.10,-0.22+Math.cos(th)*0.10,0.155];
 prev=report('orb'+th,T,[th,...prev.slice(1)],10);}
// mid-air continuity check between waypoints
console.log('--- interp aim check (lerp waypoints) ---');
const wps=[];
for(const th of[-0.45,-0.225,0,0.225,0.45]){const T=[Math.sin(th)*0.10,-0.22+Math.cos(th)*0.10,0.155];
 wps.push(solve(T,[th,...home.slice(1)]).best);}
for(let s=0;s<4;s++)for(const f of[0.25,0.5,0.75]){
 const q=wps[s].map((v,i)=>v+(wps[s+1][i]-v)*f);const{dist,aim}=info(q);
 console.log('seg'+s+' f='+f,' aim',(aim*180/Math.PI).toFixed(1),'° dist',(dist*100).toFixed(1),'cm');}
