...
function solve(T,seed,lock={}){let best=seed.map((v,i)=>Math.min(LIM[i][1],Math.max(LIM[i][0],v))),bc=cost(best,T),step=0.35;
...
const far  =report('far',  [0,-0.125,0.205],[0,0.3,-1.2,1.1,0,0.35]);
const far2 =report('far2', [0,-0.14,0.24],[0,0.5,-1.2,1.0,0,0.35]);
const top  =report('top',  [0,-0.20,0.235],[0,1.0,-0.9,0.4,0,0.35]);
const low  =report('low',  [0,-0.17,0.075],[0,0.4,-0.9,1.0,0,0.35]);
...
for(const th of[-0.6,-0.3,0,0.3,0.6]){
 ...
 report('orb'+th,[Tx,Ty,0.155],[th,...home.slice(1)],{});}
