import * as THREE from 'blob:null/FAKE-UUID-1';
import { OrbitControls } from 'blob:null/FAKE-2';
import { STLLoader } from 'blob:null/FAKE-3';
import { RoomEnvironment } from 'blob:null/FAKE-4';
...
/* ============================================================
   ...
   ============================================================ */
const CONFIG = {
  // SO-ARM100 官方网格：已内嵌本文件（base64，离线可用），以下 CDN 仅为备份
  MESH_BASES: [
...
const MAT = {
  print: new THREE.MeshStandardMaterial({ color: 0xe3e4e6, roughness: 0.52, metalness: 0.06 }),
...
async function loadArmMeshes() {
  ...
  // 网格已内嵌于本文件（base64），离线可用；CDN 仅作为内嵌缺失时的备份
  const embedded = {};
  document.querySelectorAll('script.dx-stl').forEach(el => { embedded[el.dataset.file] = el.textContent; });
  ...
  const loadOne = async file => {
    if (embedded[file]) {
      ...
      try { return loader.parse(b64ToBuf(embedded[file])); } catch (e) { /* 内嵌损坏则走 CDN */ }
    }
    for (const base of CONFIG.MESH_BASES) {
      try {
        const g = await withTimeout(loader.loadAsync(base + file), 12000);
        if (g) return g;
      } catch (e) { /* 换下一个 CDN */ }
    }
    return null;
  ...
  const tick = () => {
    ...
    veilFill.style.width = Math.round(done / total * 100) + '%';
    veilSub.textContent = '正在加载 SO-ARM100 模型… ' + done + ' / ' + total;
  };
  const jobs = [];
...
function renderPlanCard(plan) {
  ...
  wrap.innerHTML = '<div class="who">DirectorX AI · 摄影方案</div>'
    ...
    + '<button class="btn-accent p-exec"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5.5v13l11-6.5z"/></svg>执行方案</button></div></div>';
...
async function init() {
  ...
  try {
    ...
    const status = await Promise.race([loadArmMeshes(), sleep(40000).then(() => 'timeout')]);
